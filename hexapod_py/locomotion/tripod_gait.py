...
class TripodGait(Gait):
    ...
    def run(self, vx, vy, omega, roll, pitch, speed, default_foot_positions, body_height, step_height):
        ...
        for leg_idx in self.tripod_legs_1:
            joint_angles[leg_idx] = self._calculate_leg_ik(leg_idx, phase_tripod_1, vx, vy, omega, roll, pitch, default_foot_positions, self.step_length, body_height, step_height)
        for leg_idx in self.tripod_legs_2:
            joint_angles[leg_idx] = self._calculate_leg_ik(leg_idx, phase_tripod_2, vx, vy, omega, roll, pitch, default_foot_positions, self.step_length, body_height, step_height)
