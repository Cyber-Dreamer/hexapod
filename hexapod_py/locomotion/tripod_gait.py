...
class TripodGait(Gait):
    def __init__(self, kinematics, step_height, max_step_length, knee_direction):
    ...
    def run(self, vx, vy, omega, roll, pitch, speed, default_foot_positions, default_joint_angles, body_height, step_height, max_step_length):
        ...
        for leg_idx in self.tripod_legs_1:
            joint_angles[leg_idx] = self._calculate_leg_ik(leg_idx, phase_tripod_1, vx, vy, omega, roll, pitch, default_foot_positions, default_joint_angles, max_step_length, body_height, step_height)
        for leg_idx in self.tripod_legs_2:
            joint_angles[leg_idx] = self._calculate_leg_ik(leg_idx, phase_tripod_2, vx, vy, omega, roll, pitch, default_foot_positions, default_joint_angles, max_step_length, body_height, step_height)
